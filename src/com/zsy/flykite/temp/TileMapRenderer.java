...
public class TileMapRenderer implements Disposable {
	...
	public int parseIntWithDefault (String string, int defaultValue) {
	...
	/**
	 * udpate cache; create by sunqingqing
	 */
	public void udpateNormalCache(int[][][] tileMap,int layer,int pRow,int pCol){
		int row = pRow/tilesPerBlockX;
		int col = pCol/tilesPerBlockY;
//		if (isSimpleTileAtlas) {
//			// Everything considered blended
//			blendedCacheId[layer][row][col] = addBlock(tileMap[layer], row, col, false);
//		} else {
//			normalCacheId[layer][row][col] = addBlock(tileMap[layer], row, col, false);
//			blendedCacheId[layer][row][col] = addBlock(tileMap[layer], row, col, true);
//		}
		blendedCacheId[layer][row][col] = addBlock(tileMap[layer], row, col, true);
	}
	 ...
	 * udpate cache; create by sunqingqing
	 */
	public void udpateNormalCache(int layer,int pRow,int pCol,int value){
		cache.beginCache();
		
		int row = pRow/tilesPerBlockX;
		int col = pCol/tilesPerBlockY;
		
		normalCacheId[layer][row][col] =  value;
		blendedCacheId[layer][row][col] = value;
		cache.endCache();
	}
}
